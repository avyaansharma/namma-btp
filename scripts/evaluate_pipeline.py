...
if __name__ == '__main__':
    ...
    out_dir = r"c:\Users\Acer\OneDrive\Desktop\seatbelt\outputs\predictions_only"
    os.makedirs(out_dir, exist_ok=True)

    saved_belted = 0
    saved_unbelted = 0
    images = glob.glob(os.path.join(test_images_dir, "*.jpg"))
    print(f"Generating balanced clean prediction images (5 Belted, 5 Unbelted)...")

    # Class mapping for rendering
    class_names = {
        0: "Unbelted",
        1: "Belted",
        2: "Strap"
    ...
    for i, img_path in enumerate(images):
        ...
        gt_has_occupant = False
        if os.path.exists(label_path):
            with open(label_path, "r") as f:
                for line in f.readlines():
                    parts = line.strip().split()
                    if len(parts) >= 1 and int(parts[0]) in [0, 1]:
                        gt_has_occupant = True
        
        if not gt_has_occupant:
            continue
        ...
        if best_windshield is not None:
            ...
            if len(pred_boxes) > 0:
                ...
                if should_save:
                    ...
                    out_file = os.path.join(out_dir, f"prediction_{prefix}_{count}.jpg")
                    cv2.imwrite(out_file, vis_img)
                
    print(f"Successfully saved {saved_belted} belted and {saved_unbelted} unbelted pure prediction images to: {out_dir}")
