...
ROOT_DIR = os.getcwd()
org_json_path = os.path.join(ROOT_DIR, 'train_raw_fisheye8k.json')
org_json = json.load(open(org_json_path))
DAY_DIR = os.path.join(ROOT_DIR, 'sample_dataset', 'GSAD_Output', 'train')
day_image_names = os.listdir(DAY_DIR)
NAFNET_DIR = os.path.join(ROOT_DIR, 'sample_dataset', 'NAFNet_Output', 'train')
...
ROOT_DIR = os.getcwd()
org_json_path = os.path.join(ROOT_DIR, 'val_raw_fisheye8k.json')
org_json = json.load(open(org_json_path))
DAY_DIR = os.path.join(ROOT_DIR, 'sample_dataset', 'GSAD_Output', 'val')
day_image_names = os.listdir(DAY_DIR)
NAFNET_DIR = os.path.join(ROOT_DIR, 'sample_dataset', 'NAFNet_Output', 'val')
json_data = generate_final_images(org_json, DAY_DIR, day_image_names, NAFNET_DIR)
if VISUALIZE:
    image_info = random.choice(json_data['images'])
    image_file_path = image_info['file_name']
